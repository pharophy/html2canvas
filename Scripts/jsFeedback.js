...
(function ($) {
    $.fn.feedback = function (options) {
        ...
        var submitHandler = (function () {
            alert("Submit Handler Not Defined.");
        });

        //set up extension for options
        var settings = {
            'destinationImage': '#imgCanvas',
            'debugMode': true,
            'htmlForm': '',
        ...
        var cancel = $('<button />').text('Cancel').click(function() {
            $(".feedback-form").hide();
        });
        var preview = $('<button />').text('Preview').click(function () {
            ...
            canvasDom.write('Generating Preview... Please wait');
            ...
            window.setTimeout(function () {
                var canvas = $('body').html2canvas({
                    ready: function (canvas) {
                        ...
                        canvasDom.write('<h1>Send Feedback</h1>' + '<img class="imgScreenShot" src="' + a + '" style="border:1px solid black; width:400px;" />');
                        canvasDom.write('<br /><br />Message:<br /><textarea class="tbxMessage" style="width:400px;height:200px;">' + textMessage.val() + '</textarea>');
                        canvasDom.write('<br /><button id="btnSubmit">Submit</button>');
                        canvasDom.write('<script type="text/javascript" src="http://code.jquery.com/jquery-1.4.2.min.js"></script>');
                        canvasDom.write('<script type="text/javascript"> callback = ' + settings.submitHandler + '; $("#btnSubmit").live("click", function () { callback(); });</script>');
        ...
        .append(
            $('<p />').text('Left click and drag mouse on screen to highlight problem areas.')
